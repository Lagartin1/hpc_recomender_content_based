...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Prueba para Patagon con Amazon Reviews 2023 Electronics.",
    )
    ...
    parser.add_argument(
        "--configs",
        default="10000:50000,50000:100000,100000:200000",
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Maximo de bloques Ray simultaneos. Reduce memoria por worker en Patagon.",
    )
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--sample-interval", type=float, default=0.05)
    parser.add_argument(
        "--backend",
        choices=["numpy", "torch_gpu", "ray_cpu", "ray_cuda"],
        help="Ejecuta solo un backend. Si se omite, usa --backends.",
    )
    parser.add_argument(
        "--backends",
    ...
    parser.add_argument("--csv", default="results/amazon_patagon_benchmark.csv")
...
def main() -> None:
    args = parse_args()
    rows = []
    backends = [args.backend] if args.backend else parse_backends(args.backends)
    ...
    if any(backend.startswith("ray") for backend in backends):
        import ray
    ...
    try:
        ...
        if args.save_recommendations:
            output_path = str(
                ...
                / f"amazon_patagon_{backend}_u{len(dataset.user_ids)}_i{len(dataset.item_ids)}.npz"
            )
